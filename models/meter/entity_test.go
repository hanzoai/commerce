...
// which shadowed that method, so *Meter quietly stopped satisfying mixin.Entity.
//
...
//
// This is that question, asked at build time. `var _ mixin.Entity = (*Meter)(nil)`
...
func TestMeterIsAnEntity(t *testing.T) {
	if _, ok := any(&Meter{}).(mixin.Entity); !ok {
		t.Fatal("*Meter no longer satisfies mixin.Entity — a field is shadowing one of its methods " +
			...
			"Build with `var _ mixin.Entity = (*Meter)(nil)` to have the compiler name it.")
	}
}
