...
type Meter struct {
	...
	//
	// Product and Meter are kept beside it as the parts, because an editor groups
	...
	// name, a machine size. (Product, Meter) is the identity of a rate.
	//
	...
	// Label is what a person reads. Meter is what a system matches.
...
func (m *Meter) Listed() bool { return !strings.EqualFold(m.Status, "archived") }
...
func (m *Meter) Bind() { m.Slug = m.Product + "/" + m.Meter }
...
func Query(db *datastore.Datastore) datastore.Query { return db.Query("meter") }
