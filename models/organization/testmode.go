...
// Born is the kind a NEW org's record is created in, and only that: it is read
// once, as the record is first written, and never again. Every read after that
// is the record's own Live, so moving an existing org between kinds stays the
// one explicit act it was.
//
// The deployment declares it, because only the deployment knows whether real
// money exists where it runs. Undeclared, it is Test: a commerce that says
// nothing about its world creates sandbox tenants, the fail-closed direction
// TestMode describes. A host whose network exists to take real money declares
// Live (EmbedConfig.Born), and its customers are born able to pay.
var Born = Test
