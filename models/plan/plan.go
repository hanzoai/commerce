...
type Plan struct {
	...
	// months and rounded to the cent, so it is a price tag and never a figure to
	// bill from — $199 / 12 is $16.5833…, and twelve rounded months is $198.96.
	// Zero means the plan is not sold by the year.
	AnnualTotal     currency.Cents `json:"annualTotal"`
	...
	// ACTIVE (sold and listed), DRAFT (exists, not yet listed), PRIVATE (made for
	// one customer: never listed, assigned by staff) or ARCHIVED (retired, kept).
	// Only ACTIVE rows reach the public catalog.
	//
...
const (
	...
	StatusPrivate  = "private"  // made for one customer: not listed, assigned by staff
...
// moment this ships. Only an explicit draft, private or archived hides a row.
func (p *Plan) Listed() bool {
	return p.Status == "" || p.Status == StatusActive
}

// Assignable reports whether STAFF may put a customer on this plan: everything
// Listed, and a private plan besides. A private plan is one made for a single
// customer — a price agreed with them — so it is never offered, and no self-serve
// path may open it: each of those asks Listed. The staff path, which records a
// plan paid outside checkout, asks this.
func (p *Plan) Assignable() bool {
	return p.Listed() || p.Status == StatusPrivate
}
...
func Seed(db *datastore.Datastore, rows []*Plan) (created, corrected int, err error) {
	...
	for _, row := range all {
		...
		// An archived row is sold at no level. Renewals bill the subscription's own
		// snapshot, so the ladder here prices nothing anyone holds; keeping it would
		// leave a price list for a plan that is not for sale.
		existing.Prices = nil
		if uerr := existing.Update(); uerr != nil {
			return created, corrected, uerr
		}
...
func planEqual(a, b *Plan) bool {
	if a.Name != b.Name || a.Description != b.Description || a.Category != b.Category ||
		a.Price != b.Price || a.PriceAnnual != b.PriceAnnual || a.AnnualTotal != b.AnnualTotal || a.Currency != b.Currency ||
...
func copyInto(dst, src *Plan) {
	...
	dst.AnnualTotal = src.AnnualTotal
