...
func TestSeed_ArchivesWhatTheCatalogStoppedPublishing(t *testing.T) {
	...
	rows := sampleRows()
	rows[1].Prices = []currency.Cents{2500, 5000, 7500} // a ladder the archive must drop
	...
	for _, p := range all {
		...
		// It is sold at no level: the ladder goes with the listing.
		if len(p.Prices) != 0 {
			t.Errorf("archived %q still publishes a ladder %v", p.Slug, p.Prices)
		}
	}
}
