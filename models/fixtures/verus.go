...
var Verus = New("verus", func(c *gin.Context) *organization.Organization {
	...
	u6 := user.New(db)
	u6.Email = "thomas@verus.io"
	u6.GetOrCreate("Email=", u6.Email)
	u6.FirstName = "Thomas"
	u6.LastName = "Noe"
	u6.Organizations = []string{org.Id()}
	u6.PasswordHash, _ = password.Hash("veruspassword!")
	u6.MustUpdate()

	// Configure org
	org.FullName = "verus"
	org.Owners = []string{u.Id(), u2.Id(), u3.Id(), u4.Id(), u5.Id(), u6.Id()}
	...
	org.ResetReferenceToken(u, claims)
	org.ResetReferenceToken(u2, claims)
	org.ResetReferenceToken(u3, claims)
	org.ResetReferenceToken(u4, claims)
	org.ResetReferenceToken(u5, claims)
	org.ResetReferenceToken(u6, claims)
