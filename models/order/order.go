...
type Order struct {
	...
	// Associated Hanzo user or buyer.
	...
	CouponCodes_ string          `json:"-"` // need props
	ReferrerId   string          `json:"referrerId,omitempty"`

	PaymentIds  []string `json:"payments"`
	PaymentIds_ string   `json:"-"'` // need props
...
func (o Order) GetPayments() ([]*payment.Payment, error) {
	slice, err := payment.Query(o.Db).Ancestor(o.Key()).GetAll()

	if err != nil {
		return nil, err
	}

	return slice.([]*payment.Payment), nil
}
