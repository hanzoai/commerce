...
The `DataTable` component is useful if you're displaying large data with functionalities like pagination, filtering, sorting, and searching. It's also the recommended table component to use when creating customizations in the Hanzo Commerce Admin.

<Note>

This component is available after Hanzo Commerce UI v4.0.4 (or Hanzo Commerce v2.4.0). It is built on top of the [Table](../table/page.mdx) component. If you want a table with more control over its styling and functionality, use that component instead.
...
You import the `DataTable` component from `@hanzo/commerce-ui`.
...
The `DataTable` component expects a table instance created using the `useDataTable` hook. Import that hook from `@hanzo/commerce-ui`:
...
- `rowCount`: The total number of rows in the table. If you're fetching data from the Hanzo Commerce application with pagination or filters, this will be the total count, not the count of the data returned in the current page.
- `isLoading`: A boolean that indicates whether the table is loading data. This is useful when loading data from the Hanzo Commerce application for the first time or in between pages.
...
Refer to [this Admin Components guide](https://docs.hanzo.ai/resources/admin-components/components/data-table) for an example on using the `DataTable` component with data fetching from the Hanzo Commerce application.
...
Next, you must implement the search filtering. For example, if you're retrieving data from the Hanzo Commerce application, you pass the search query to the API to filter the data.
...
First, import the `DataTablePaginationState` type from `@hanzo/commerce-ui`:
...
You must also implement the pagination logic, such as fetching data from the Hanzo Commerce application with the pagination parameters.
...
First, add the following imports from the `@hanzo/commerce-ui` package:
...
You must also implement the logic of filtering the data based on the filter conditions, such as sending the filter conditions to the Hanzo Commerce application when fetching data.
...
You must also implement the sorting logic, such as sending the sorting conditions to the Hanzo Commerce application when fetching data.
...
First, add the following imports from `@hanzo/commerce-ui`:
