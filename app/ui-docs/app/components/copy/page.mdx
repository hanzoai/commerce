...
```tsx
<Copy content="yarn add @hanzo/commerce-ui" />
```
...
If you're using the `Copy` component in a project other than the Hanzo Commerce Admin, make sure to include the `TooltipProvider` somewhere up in your component tree, as the `Copy` component uses a [Tooltip](../tooltip/page.mdx#usage-outside-hanzo-commerce-admin):
