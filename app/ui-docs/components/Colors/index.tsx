...
import { CopyButton, H2, Hr, MarkdownContent, useColorMode } from "@hanzo/commerce-docs-ui"
...
const ColorBlock = ({ colour, className, ...props }: ColorBlockProps) => {
  ...
  if (!mounted) {
    return (
      <div className="flex w-fit flex-row items-center gap-x-2">
        <div
          className={
            "border-hanzo-border-base h-[48px] w-[48px] rounded-lg border p-1"
          }
        >
          <div
            className={clx(
              "bg-hanzo-bg-component h-full w-full animate-pulse rounded-[4px]",
              className
            )}
            {...props}
          />
        ...
        <div className="flex flex-col items-start">
          <div className="bg-hanzo-bg-component h-[20px] w-[85px] animate-pulse rounded-sm" />
          <div className="bg-hanzo-bg-subtle h-[20px] w-[120px] animate-pulse rounded-sm" />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-row items-center gap-x-2">
      ...
      <div className="flex flex-col items-start">
        <p className="txt-compact-xsmall-plus text-hanzo-fg-basetext-start">
          {cssVarToTailwindClass(colour.name)}
        </p>
        <p className="txt-compact-xsmall text-hanzo-fg-subtle">
          {colour.code}
        </p>
      </div>
    </div>
