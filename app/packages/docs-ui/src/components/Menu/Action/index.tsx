    ...
    <div className="px-docs_0.25">
      <span
        className={clsx(
          ...
          "hover:bg-hanzo-bg-component-hover",
          "text-hanzo-fg-base cursor-pointer"
        )}
        tabIndex={-1}
        onClick={() => {
          item.action()
      ...
      >
        <span className="text-hanzo-fg-subtle mt-[2.5px] block">
          {item.icon}
        </span>
        <span className="text-compact-small flex-1">{item.title}</span>
        {item.shortcut && (
          <span
            className="text-hanzo-fg-subtle text-compact-small"
