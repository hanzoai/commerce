...
export const VerticalCodeTabs = ({
  ...
  return (
    <div
      className={clsx(
        "rounded-[20px] bg-hanzo-bg-subtle p-docs_0.5",
    ...
    >
      <div
        className={clsx(
          "rounded-docs_lg bg-hanzo-contrast-bg-base h-full",
          ...
          {new Array(3).fill(0).map((_, index) => (
            <span
              className={clsx(
                ...
                "bg-hanzo-contrast-border-bot border border-hanzo-contrast-border-bot"
              )}
              key={index}
            ...
            {tabs.map((tab, index) => (
              <li
                className={clsx(
                  ...
                  "rounded-docs_DEFAULT border border-hanzo-contrast-border-bot",
                  "text-code-body font-monospace cursor-pointer",
                  selectedTabIndex === index &&
                    "text-hanzo-contrast-fg-primary bg-hanzo-contrast-border-bot",
                  selectedTabIndex !== index &&
                    "text-hanzo-contrast-fg-secondary bg-hanzo-contrast-bg-subtle hover:bg-hanzo-contrast-border-bot"
                )}
          ...
          <div className="flex-1 h-[388px] max-w-full overflow-hidden rounded-docs_DEFAULT [&_pre]:min-h-full">
            <CodeBlock
              ...
              className={clsx(
                ...
                "!border !border-hanzo-contrast-border-bot"
              )}
              innerClassName="h-full"
              animateTokenHighlights
              overrideColors={{
                bg: "bg-hanzo-contrast-bg-subtle",
                innerBg: "bg-hanzo-contrast-bg-subtle",
                lineNumbersBg: "bg-hanzo-contrast-bg-subtle",
