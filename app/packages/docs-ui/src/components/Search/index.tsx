  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <InstantSearch
        ...
        insights={true}
      >
        <div className={clsx("bg-hanzo-bg-base flex z-[1]")}>
          {/* @ts-expect-error React v19 doesn't see this type as a React element */}
          <SearchBox
            classNames={{
              root: clsx(
                "h-[57px] w-full md:rounded-t-docs_xl relative border-0 border-solid",
                "border-b border-hanzo-border-base",
                "bg-transparent"
              ),
              form: clsx("h-full md:rounded-t-docs_xl bg-transparent"),
              input: clsx(
                "w-[calc(100%-40px)] h-full px-docs_1 py-docs_0.75 text-hanzo-fg-base",
                "placeholder:text-hanzo-fg-muted bg-hanzo-bg-base",
            ...
            submitIconComponent={() => <></>}
            resetIconComponent={() => (
              <span className="text-hanzo-fg-muted text-compact-small-plus hover:text-hanzo-fg-subtle">
                Clear
              </span>
            )}
