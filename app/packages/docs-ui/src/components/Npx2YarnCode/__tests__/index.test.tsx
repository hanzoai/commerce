...
describe("render", () => {
  test("renders npm2yarn code", () => {
    const { container } = render(
      <Npx2YarnCode npxCode="npx hanzo-commerce db:migrate" />
    )
    ...
    expect(npxCodeBlock).toHaveTextContent("npx hanzo-commerce db:migrate")
  })

  test("renders npm2yarn code with custom code options", () => {
    const { container } = render(
      <Npx2YarnCode
        npxCode="npx hanzo-commerce db:migrate"
        title="Custom Title"
      />
    )
    expect(container).toBeInTheDocument()
  ...
  test("calls npxToYarn with isExecutable=false by default", () => {
    ...
    render(<Npx2YarnCode npxCode="npx hanzo-commerce db:migrate" />)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx hanzo-commerce db:migrate", "yarn", false)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx hanzo-commerce db:migrate", "pnpm", false)
  })

  test("calls npxToYarn with isExecutable=true when specified", () => {
    npxToYarnMock.mockClear()
    render(<Npx2YarnCode npxCode="npx create-hanzo-app@latest" isExecutable={true} />)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx create-hanzo-app@latest", "yarn", true)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx create-hanzo-app@latest", "pnpm", true)
  })

  test("calls npxToYarn with isExecutable=false when explicitly set", () => {
    npxToYarnMock.mockClear()
    render(<Npx2YarnCode npxCode="npx hanzo-commerce db:migrate" isExecutable={false} />)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx hanzo-commerce db:migrate", "yarn", false)
    expect(npxToYarnMock).toHaveBeenCalledWith("npx hanzo-commerce db:migrate", "pnpm", false)
  })
})
