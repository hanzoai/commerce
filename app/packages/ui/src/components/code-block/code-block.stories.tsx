...
const snippets = [
  ...
  {
    label: "Hanzo Commerce JS Client",
    language: "jsx",
    code: `// Install the JS Client in your storefront project: @hanzo/commerce-js\n\nimport Commerce from "@hanzo/commerce-js"\n\nconst commerce = new Commerce({ publishableApiKey: "YOUR_API_KEY"})\nconst product = await commerce.products.retrieve("PRODUCT_ID")\nconsole.log(product.id)`,
  },
  {
    label: "Hanzo Commerce React",
    language: "tsx",
    code: `// Install the React SDK and required dependencies in your storefront project:\n// @hanzo/commerce-react @tanstack/react-query @hanzo/commerce\n\nimport { useProduct } from "@hanzo/commerce-react"\n\nconst { product } = useProduct("PRODUCT_ID")\nconsole.log(product.id)`,
  },
]
