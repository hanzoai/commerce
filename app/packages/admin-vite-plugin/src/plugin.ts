...
import { HanzoCommerceVitePlugin } from "./types"
...
export const hanzoCommerceVitePlugin: HanzoCommerceVitePlugin = (options) => {
  ...
  async function generatePluginEntryModule(
    ...
    return `
      // Auto-generated index file for Hanzo Commerce Admin UI extensions
    ${widgetModule.code}
    ${routeModule.code}
    ${menuItemModule.code}
    ${formModule.code}
  ...
  return {
    name: "@hanzo/commerce-admin-vite-plugin",
    enforce: "pre",
    async buildStart() {
      switch (mode) {
        case Mode.PLUGIN: {
