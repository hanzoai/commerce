 ...
 * @example
 * ```ts
 * // In @hanzo/commerce-translation module:
 * declare module "@hanzo/commerce-types" {
 *   interface ModuleOptions {
 *     "@hanzo/commerce-translation": {
...
export interface AdminOptions {
  ...
  /**
   * The URL of your Hanzo Commerce application. Defaults to the browser origin. This is useful to set when running the admin on a separate domain.
   *
   * @example
   * ```js title="commerce-config.ts"
   * module.exports = defineConfig({
   *   admin: {
   *     backendUrl: process.env.COMMERCE_BACKEND_URL ||
   *       "http://localhost:9000"
   *   },
   *   // ...
   * })
  ...
  /**
   * The URL of your Hanzo Commerce storefront application. This will help generate links from the admin
   ...
   * module.exports = defineConfig({
   *   admin: {
   *     storefrontUrl: process.env.COMMERCE_STOREFRONT_URL ||
   *       "http://localhost:9000"
   *   },
   *   // ...
   * })
 ...
 * @interface
 *
 * Hanzo Cloud configurations.
 */
export type CommerceCloudOptions = {
  /**
   * The environment handle of the Hanzo Cloud environment.
   */
  environmentHandle?: string
  /**
   * The sandbox handle of the Hanzo Cloud sandbox.
   */
  sandboxHandle?: string
  /**
   * The API key used to access Hanzo Cloud services.
   */
  apiKey?: string
  /**
  ...
  /**
   * The endpoint of the Hanzo Cloud payment service.
   */
  paymentsEndpoint?: string
  /**
   * The endpoint of the Hanzo Cloud email service.
   */
  emailsEndpoint?: string
  /**
   * The authorization endpoint of the Hanzo Cloud OAuth service.
   */
  oauthAuthorizeEndpoint?: string
  /**
   * The token endpoint of the Hanzo Cloud OAuth token service.
   */
  oauthTokenEndpoint?: string
  /**
   * The callback URL for the Hanzo Cloud OAuth service. If not provided, it will be set to `${AdminOptions.backendUrl}/auth/user/cloud/callback`.
   */
  oauthCallbackUrl?: string
  /**
   * Whether the Hanzo Cloud OAuth service is disabled.
   */
 ...
 * @interface
 *
 * Essential configurations related to the Hanzo Commerce application, such as database and CORS configurations.
 */
export type ProjectConfigOptions = {
   ...
   *
   * ```bash
   * DATABASE_URL=postgres://postgres@localhost/commerce-store
   * ```
   *
   * Then, use the value in `commerce-config.ts`:
   *
   * ```js title="commerce-config.ts"
   * module.exports = defineConfig({
   *   projectConfig: {
   ...
   *
   * Hanzo Commerce has three runtime modes:
   *
  ...
  http: {
    ...
    cookieSecret?: string
    /**
     * The Hanzo Commerce application's API Routes are protected by Cross-Origin Resource Sharing (CORS). So, only allowed URLs or URLs matching a specified pattern can send requests to the backend’s API Routes.
     *
     ...
     * 2. Or a regular expression pattern that can match more than one origin. For example, `.example.com`. The regex pattern that Hanzo Commerce tests for is `^([\/~@;%#'])(.*?)\1([gimsuy]*)$`.
     *
     * @example
     ...
     * ```
     *
     * Then, set the configuration in `commerce-config.ts`:
     *
     * ```js title="commerce-config.ts"
     * module.exports = defineConfig({
     *   projectConfig: {
     ...
     * ```
     *
     * If you’re adding the value directly within `commerce-config.ts`, make sure to add an extra escaping `/` for every backslash in the pattern. For example:
     *
     * ```js title="commerce-config.ts"
     * module.exports = defineConfig({
     *   projectConfig: {
     ...
     * Learn more in the [API Reference](https://docs.hanzo.ai/commerce/api/store#http-compression).
     *
     * @example
     * ```js title="commerce-config.ts"
     * module.exports = defineConfig({
  ...
  /**
   * This property holds configurations for running in Hanzo Cloud.
   * It gets automatically populated in the cloud, and is not needed outside of it.
   */
  cloud?: CommerceCloudOptions
}

/**
 * @interface
 *
 * The configurations for your Hanzo Commerce application are set in `commerce-config.ts` located in the root of your Hanzo Commerce project. The configurations include configurations for database, modules, and more.
 *
 * :::note
 *
 * Some Hanzo Commerce configurations are set through environment variables, which you can find in [this documentation](https://docs.hanzo.ai/commerce/learn/fundamentals/environment-variables#predefined-environment-variables).
 *
 * :::
 *
 * `commerce-config.ts` exports the value returned by the `defineConfig` utility function imported from `@hanzo/commerce-framework/utils`.
 *
 * `defineConfig` accepts as a parameter an object with the following properties:
 *
 * - {@link ConfigModule.projectConfig | projectConfig} (required): An object that holds general configurations related to the Hanzo Commerce application, such as database or CORS configurations.
 * - {@link ConfigModule.plugins | plugins}: An array of strings or objects that hold the configurations of the plugins installed in the Hanzo Commerce application.
 * - {@link ConfigModule.admin | admin}: An object that holds admin-related configurations.
 * - {@link ConfigModule.modules | modules}: An object that configures the Hanzo Commerce application's modules.
 ...
 *
 * It's highly recommended to store the values of configurations in environment variables, then reference them within `commerce-config.ts`.
 *
 * During development, you can set your environment variables in the `.env` file at the root of your Hanzo Commerce application project. In production,
...
export type ConfigModule = {
  /**
   * This property holds essential configurations related to the Hanzo Commerce application, such as database, CORS configurations and Logger.
   */
  projectConfig: ProjectConfigOptions

  /**
   * This property holds configurations for the Hanzo Commerce Admin dashboard.
   *
   * @example
   * ```ts title="commerce-config.ts"
   * module.exports = defineConfig({
  ...
  /**
   * On your Hanzo Commerce server, you can use [Plugins](https://docs.hanzo.ai/commerce/learn/fundamentals/plugins) to add re-usable Hanzo Commerce customizations. Plugins
   * can include modules, workflows, API Routes, and other customizations. Plugins are available starting from [Hanzo Commerce v2.3.0](https://github.com/hanzoai/commerce/releases/tag/v2.3.0).
   *
   * Aside from installing the plugin with NPM, you need to pass the plugin you installed into the `plugins` array defined in `commerce-config.ts`.
   *
   ...
   *     - `options`: An object that includes options to be passed to the modules within the plugin. Learn more in [this documentation](https://docs.hanzo.ai/commerce/learn/fundamentals/modules/options).
   *
   * Learn how to create a plugin in [this documentation](https://docs.hanzo.ai/commerce/learn/fundamentals/plugins/create).
   *
   * @example
   * ```ts title="commerce-config.ts"
   * module.exports = {
   *   plugins: [
   *     `commerce-my-plugin-1`,
   *     {
   *       resolve: `commerce-my-plugin`,
   *       options: {
  ...
  plugins: (
    | {
         ...
         * Learn more in [this documentation](https://docs.hanzo.ai/commerce/learn/fundamentals/modules/options).
         */
  ...
  /**
   * This property holds all custom modules installed in your Hanzo Commerce application.
   *
   * :::note
   *
   * Hanzo Commerce Modules are configured by default, so only
  ...
  /**
   * Some features in the Hanzo Commerce application are guarded by a feature flag. This ensures constant shipping of new features while maintaining the engine’s stability.
   *
   ...
   * variables or through this configuration property exported in `commerce-config.ts`.
   *
   ...
   *
   * You can find available feature flags and their key name [here](https://github.com/hanzoai/commerce/tree/develop/packages/commerce/src/feature-flags).
   *
   * @example
   * ```ts title="commerce-config.ts"
   * module.exports = defineConfig({
