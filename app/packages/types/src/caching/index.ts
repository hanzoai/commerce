...
type Providers = (string | { 
  /**
   * The ID of the provider to use, as set in `commerce-config.ts`.
   */
  id: string;
  /**
...
export interface ICachingModuleService extends IModuleService {
  ...
  get({
     ...
     * If not provided, the [default provider](https://docs.hanzo.ai/commerce/infrastructure-modules/caching/providers#default-caching-module-provider) is used.
     */
    providers?: string[]
  }): Promise<any | null>
   ...
   * [default Caching Module Provider](https://docs.hanzo.ai/commerce/infrastructure-modules/caching/providers#default-caching-module-provider).
   *
   ...
   * :::note
   * 
   * Tags should follow [conventions](https://docs.hanzo.ai/commerce/infrastructure-modules/caching/concepts#caching-tags-convention) to ensure they're automatically invalidated.
   * 
   * :::
   * 
   * ```ts
 ...
 *
 * If you're creating a client or establishing a connection with a third-party service, do it in a [Loader](https://docs.hanzo.ai/commerce/learn/fundamentals/modules/loaders)
 ...
 *
 * ```ts
 * import { ICachingProviderService } from "@hanzo/commerce-types"
 * import { Logger } from "@hanzo/commerce-types"
 *
 ...
 * will be stored as `lp_{identifier}_{id}`, where `id` is the ID you set in your `commerce-config.ts` file.
 * 
 * For example:
 * 
 * ```ts
...
export interface ICachingStrategy {
   ...
   *
   * @param container CommerceContainer
