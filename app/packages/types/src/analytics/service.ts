...
export interface IAnalyticsModuleService extends IModuleService {
  /**
   * This method returns the service of the configured Analytics Module Provider in `commerce-config.ts`. This is useful
   ...
   * will use the `track` method of the underlying provider configured in `commerce-config.ts` to track the event.
   *
   ...
   * will use the `identify` method of the underlying provider configured in `commerce-config.ts` to identify the actor or group.
   *
