...
export interface BaseProductVariant {
  ...
  /**
   * Whether Hanzo Commerce manages the variant's inventory. If disabled, the variant
   * is always considered in stock.
   */
  manage_inventory: boolean | null
  /**
   * The variant's inventory quantity if `manage_inventory` is enabled.
   * This field is only retrieved in the [Get Product](https://docs.hanzo.ai/commerce/api/store#products_getproductsid)
   * and [List Products](https://docs.hanzo.ai/commerce/api/store#products_getproducts) API routes if you
   * pass `+variants.inventory_quantity` in the `fields` query parameter.
   *
   * Learn more in the [Retrieve Product Variant's Inventory](https://docs.hanzo.ai/commerce/resources/storefront-development/products/inventory) storefront guide.
   */
