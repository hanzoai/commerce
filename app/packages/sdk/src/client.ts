...
export const LOCALE_STORAGE_KEY = "hanzo_commerce_locale"
...
export class Client {
  ...
  private DEFAULT_JWT_STORAGE_KEY = "hanzo_commerce_auth_token"
  private token = ""
  ...
  setLocale(locale: string) {
    if (!window) {
      this.logger.warn(
        "setLocale is not available in the server environment. Please set the locale directly through the 'x-hanzo-locale' header."
      )
      return
    }
  ...
  protected initClient(): ClientFetch {
    ...
    this.logger.debug(
      "Initiating Hanzo Commerce client with default headers:\n",
    ...
    return async (input: FetchInput, init?: FetchArgs) => {
      ...
      if (this.locale) {
        headers.set("x-hanzo-locale", this.locale)
      }

      const customHeaders = {
        "x-hanzo-locale": this.locale,
