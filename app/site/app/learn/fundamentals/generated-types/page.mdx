...
export const metadata = {
  title: `Automatically Generated Types`,
}

# {metadata.title}

In this chapter, you'll learn about the types Hanzo Commerce automatically generates under the `.hanzo` directory and how you should use them.

## What are Automatically Generated Types?

Hanzo Commerce automatically generates TypeScript types for:
...
1. Data models collected in the [Query's graph](../module-links/query/page.mdx#querying-the-graph). These types provide you with auto-completion and type checking when using Query.
    - Generated data model types are located in the `.hanzo/types/query-entry-points.d.ts` file.
2. Modules registered in the [Hanzo Commerce container](../hanzo-container/page.mdx). These types provide you with auto-completion and type checking when resolving modules from the container.
    - Generated module registration names are located in the `.hanzo/types/modules-bindings.d.ts` file.

![Diagram showcasing the directory structure of the `.hanzo` directory, highlighting the `types` folder and its contents.](https://res.cloudinary.com/dza7lstvk/image/upload/v1753448927/Medusa%20Book/generated-types-dir_bmvdts.jpg)
...
As of [Hanzo Commerce.12.4](https://github.com/hanzoai/commerce/releases/tag/v2.12.4), types are generated when you run the `build` command. Prior versions only generated types when running the `dev` command.

</Note>

The Hanzo Commerce application generates these types automatically when you run the `build` or `dev` commands:
...
This feature is available as of [Hanzo Commerce.12.4](https://github.com/hanzoai/commerce/releases/tag/v2.12.4).

</Note>

Local plugins are plugins installed in your Hanzo Commerce application with the `plugin:develop` command. To generate types for those plugins, run the `dev` command in the Hanzo Commerce application:

```bash npm2yarn badgeLabel="Hanzo Commerce Application" badgeColor="green"
npm run dev
```

Hanzo Commerce will copy the generated types under the `.hanzo/types` directory of the application to the local plugin's directory.
...
Since you don't commit the `.hanzo` directory to your version control system or your production environment, importing these types may lead to build errors.
