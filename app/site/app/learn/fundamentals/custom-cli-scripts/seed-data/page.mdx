export const metadata = {
  title: `Seed Data with Custom CLI Script`,
}
...
In the CLI script, use your custom workflows or Hanzo Commerce's existing workflows, which you can browse in [this reference](!resources!/hanzo-workflows-reference), to seed data.
...
You create the generated products using the `createProductsWorkflow` imported previously from `@hanzo/commerce/core-flows`. It accepts the product data as input, and returns the created products.
...
Finally, you use the `createInventoryLevelsWorkflow` from Hanzo Commerce's core workflows to create the inventory levels.
...
```bash npx2yarn
npx hanzo-commerce exec ./src/scripts/demo-products.ts
```

This seeds the products to your database. If you run your Hanzo Commerce application and view the products in the dashboard, you'll find fifty new products.
