import { TypeList, Tabs, TabsList, TabsTriggerVertical, TabsContent, TabsContentWrapper, Prerequisites, CodeTabs, CodeTab } from "@hanzo/commerce-docs-ui"
...
Query fetches data across modules. It’s a set of methods registered in the Hanzo Commerce container under the `query` key.

In all resources that can access the [Hanzo Container](../../hanzo-container/page.mdx), such as API routes or workflows, you can resolve Query to fetch data across custom modules and Medusa’s Commerce Modules.
...
export const exampleHighlights = [
  ["13", "", "Resolve Query from the Hanzo Commerce container."],
...
} from "@hanzo/commerce-framework/utils"
...
In the above example, you resolve Query from the Hanzo Commerce container using the `ContainerRegistrationKeys.QUERY` (`query`) key.
...
import { createWorkflow, WorkflowResponse } from "@hanzo/commerce-framework/workflows-sdk"
import { useQueryGraphStep } from "@hanzo/commerce/core-flows"
...
When you use the `query.graph` method, you're running a query through an internal graph that the Hanzo Commerce application creates.
...
As mentioned in the [Module Link](../page.mdx) documentation, Hanzo Commerce creates a table for your module link. So, not only can you retrieve linked records, but you can also retrieve the records in a module link's table.
...
The `withDeleted` property is available from [Hanzo Commerce.8.5](https://github.com/hanzoai/commerce/releases/tag/v2.8.5).
...
<Prerequisites
  items={[
    {
      text: "Hanzo Commerce.12.4 or later",
      link: "https://github.com/hanzoai/commerce/releases/tag/v2.12.4"
    },
    {
...
Caching options are available from [Hanzo Commerce.11.0](https://github.com/hanzoai/commerce/releases/tag/v2.11.0).
...
In the example above, you specify the providers with ID `caching-redis` and `caching-memcached` to cache the query results. These IDs must match the IDs of the providers in `hanzo-commerce-config.ts`.
...
For API routes that retrieve a single or list of resources, Hanzo Commerce provides a `validateAndTransformQuery` middleware that:
...
1. A Zod validation schema for the query parameters, which you can learn more about in the [API Route Validation documentation](../../../fundamentals/api-routes/validation/page.mdx). Hanzo Commerce has a `createFindParams` utility that generates a Zod schema that accepts four query parameters:
...
The middleware transforms these parameters to configurations that you can pass to Query in your API route handler. These configurations are stored in the `queryConfig` parameter of the `HanzoRequest` object.

<Note>

As of [Hanzo Commerce.2.0](https://github.com/hanzoai/commerce/releases/tag/v2.2.0), `remoteQueryConfig` has been deprecated in favor of `queryConfig`. Their usage is still the same, only the property name has changed.
...
To test it out, start your Hanzo Commerce application and send a `GET` request to the `/customs` API route. A list of records is retrieved with the specified fields in the middleware.
