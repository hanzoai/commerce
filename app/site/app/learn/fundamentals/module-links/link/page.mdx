import { CodeTab, CodeTabs } from "@hanzo/commerce-docs-ui"
...
As of [Hanzo Commerce.2.0](https://github.com/hanzoai/commerce/releases/tag/v2.2.0), Remote Link has been deprecated in favor of Link. They have the same usage, so you only need to change the key used to resolve the tool from the Hanzo Commerce container as explained below.

</Note>

## What is Link?

Link is a class with utility methods to manage links between data models. It’s registered in the Hanzo Commerce container under the `link` registration name.
...
To perform link operations in a [workflow](../../workflows/page.mdx), Hanzo Commerce provides the following steps out-of-the-box:
...
```ts
import { Modules } from "@hanzo/commerce-framework/utils"
import { createRemoteLinkStep } from "@hanzo/commerce/core-flows"
import { 
  createWorkflow,
  transform,
...
Hanzo Commerce enforces integrity constraints on links based on the link's relation type. So, an error is thrown in the following scenarios:
...
```ts
import { Modules } from "@hanzo/commerce-framework/utils"
import { dismissRemoteLinkStep } from "@hanzo/commerce/core-flows"
import { 
  createWorkflow,
  transform,
} from "@hanzo/commerce-framework/workflows-sdk"
...
```ts
import { Modules } from "@hanzo/commerce-framework/utils"
import { removeRemoteLinkStep } from "@hanzo/commerce/core-flows"
import { 
  createWorkflow,
  transform,
} from "@hanzo/commerce-framework/workflows-sdk"
...
```ts
import { Modules } from "@hanzo/commerce-framework/utils"
import { updateRemoteLinksStep } from "@hanzo/commerce/core-flows"
import { 
  createWorkflow,
  transform,
} from "@hanzo/commerce-framework/workflows-sdk"
