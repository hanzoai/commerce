export const metadata = {
  title: `JSON Properties in Data Models`,
}
...
Most data models in Hanzo Commerce's Commerce Modules have a `metadata` property that is a JSON object. `metadata` allows you to store custom information that is not part of the core data model.
...
### How Hanzo Commerce Updates JSON Properties

Consider a Brand Module with a `Brand` data model as shown [in the previous section](#what-is-a-json-property). The [module's service](../../modules/page.mdx#2-create-service) will extend `HanzoService`, which generates methods like [updateBrands](!resources!/service-factory-reference/methods/update) to update a brand.

When you pass a JSON property in the `updateBrands` method, Hanzo Commerce will merge the provided JSON object with the existing one in the database. So, only the properties you pass will be updated, and the rest will remain unchanged.
...
The following examples assume you have a `brandModuleService` that is resolved from the [Hanzo Commerce container](../../hanzo-container/page.mdx). For example:
