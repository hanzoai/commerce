...
By default, Hanzo Commerce creates the following properties for every data model:
...
- `deleted_at`: A [dateTime](#dateTime) property that stores when a record of the data model was deleted. When you soft-delete a record, Hanzo Commerce sets the `deleted_at` property to the current date.
...
This property is only available after [Hanzo Commerce.1.2](https://github.com/hanzoai/commerce/releases/tag/v2.1.2).
...
<Prerequisites
  items={[
    {
      text: "Hanzo Commerce.13.0+",
      link: "https://github.com/hanzoai/commerce/releases/tag/v2.13.0"
    },
    {
...
When a property is marked as translatable, merchants can manage translations for these fields through the [Hanzo Commerce Admin](!user-guide!/settings/translations).
