...
In this chapter, you'll learn how to add translations to your Hanzo Commerce Admin widgets and UI routes.

<Note>

Translations for admin customizations are available from [Hanzo Commerce.11.1](https://github.com/hanzoai/commerce/releases/tag/v2.11.1).

</Note>

## Translations in the Hanzo Commerce Admin

The Hanzo Commerce Admin dashboard supports [multiple languages](!user-guide!/tips/languages) for its interface. Hanzo Commerce uses [react-i18next](https://react.i18next.com/) to manage translations in the admin dashboard.

<Note>

Hanzo Commerce Admin translations apply to the interface only. To translate content like product data, check out the [Translations user guide](!user-guide!/settings/translations).

</Note>

When you create [Widgets](../widgets/page.mdx) or [UI Routes](../ui-routes/page.mdx) to customize the Hanzo Commerce Admin, you can provide translations for the text content in your customizations, allowing users to view your customizations in their preferred language.

You can add translations for your admin customizations within your Hanzo Commerce project or as part of a plugin.
...
English is the default language for Hanzo Commerce Admin, so it's recommended to always include an English translation file.

</Note>

![Directory structure showing where to place translation files](/images/learn/translations-json_m2pvet.jpg)
...
![Directory structure showing where to place translation files](/images/learn/translations-json-es_s1etna.jpg)
...
![Directory structure showing i18n index file](/images/learn/translations-index_cgrj0t.jpg)
...
The `react-i18next` package is already included in the Hanzo Commerce Admin's dependencies, so you don't need to install it separately. However, `pnpm` users may need to install it manually due to package resolution issues.
...
To test the translations, start the Hanzo Commerce application with the following command:

```bash npm2yarn
npm run dev
```

Then, go to a product details page in the Hanzo Commerce Admin dashboard. If your default language is set to English, you'll see the widget displaying text in English.
...
Translation auto-completion is available from [Hanzo Commerce.11.2](https://github.com/hanzoai/commerce/releases/tag/v2.11.2).

</Note>

To enhance your development experience, you can set up auto-completion for translation keys. This will allow you to auto-complete translation keys from Hanzo Commerce's default translations as well as your custom translations.
...
// Import Hanzo Commerce keys
import type { Resources } from "@hanzo/commerce-dashboard"
...
When you load the translations with the `translation` key in `src/admin/i18n/index.ts`, your custom Hanzo Commerce Admin translations are merged with the default Hanzo Commerce Admin translations:

- Translation keys in your custom translations override the default Hanzo Commerce Admin translations.
- The default Hanzo Commerce Admin translations are used as a fallback when a key is not defined in your custom translations.
...
- `brands.description`: Not defined in your custom translation file or the default Hanzo Commerce Admin translations, so it outputs the key itself: `brands.description`.
- `actions.save`: Defined in your custom translation file, so it outputs `Custom Save`.
- `actions.delete`: Not defined in your custom translation file, so it falls back to the default Hanzo Commerce Admin translation and outputs `Delete`.

### Custom Translation Namespaces

To avoid potential key conflicts between your custom translations and the default Hanzo Commerce Admin translations, you can use custom namespaces. This is particularly useful when developing plugins that add admin customizations, as it prevents naming collisions with other plugins or the default Hanzo Commerce Admin translations.
...
Translations are now loaded only from the `brands` namespace without conflicting with other translation keys in the Hanzo Commerce Admin.
