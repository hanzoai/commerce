export const metadata = {
  title: `Locking Operations in Workflows`,
}
...
Locks are particularly useful in workflows where concurrent executions may lead to commerce risks. For example, Hanzo Commerce uses locks in its cart workflows to prevent issues like overselling products or double charging customers.
...
Hanzo Commerce provides two steps that you can use to create locks in your workflows:

- [acquireLockStep](!resources!/references/hanzo-workflows/steps/acquireLockStep): Attempt to acquire a lock. If the lock is already held by another process, this step will wait until the lock is released. It will fail if a timeout occurs before acquiring the lock.
- [releaseLockStep](!resources!/references/hanzo-workflows/steps/releaseLockStep): Release a previously acquired lock.
...
import { acquireLockStep, releaseLockStep } from "@hanzo/commerce/core-flows"
...
Refer to the [acquireLockStep](!resources!/references/hanzo-workflows/steps/acquireLockStep) and [releaseLockStep](!resources!/references/hanzo-workflows/steps/releaseLockStep) for more information on the inputs of these steps.
...
import { createStep } from "@hanzo/commerce-framework/workflows-sdk"
...
For example, consider a custom workflow that executes Hanzo Commerce's [completeCartWorkflow](!resources!/references/hanzo-workflows/completeCartWorkflow). The `completeCartWorkflow` acquires locks on the cart with `acquireLockStep` to prevent race conditions.
