...
So, resources in the module, such as services or loaders, can only resolve other resources registered in the module's container, and some Framework tools that the Hanzo Commerce application registers in the module's container.

### List of Registered Resources

Find a list of resources or dependencies registered in a module's container in [the Container Resources reference](!resources!/hanzo-container-resources).
...
Consider a module that has a main service `BrandModuleService`, and an internal service `CmsService`. Hanzo Commerce will register both of these services in the module's container.
...
Instead, if your main service extends the `HanzoService` [service factory](../service-factory/page.mdx), you can resolve the internal services generated for each data model passed to the `HanzoService` function.
...
Instead, to build commerce features that span multiple modules, you can create [workflows](../../workflows/page.mdx). In those workflows, you can resolve services of all modules registered in the Hanzo Commerce application, including the services of the Product and Blog modules.
