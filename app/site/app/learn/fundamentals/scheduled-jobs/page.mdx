...
Medusa removes this overhead by supporting this feature natively with scheduled jobs. A scheduled job is an asynchronous function that the Hanzo Commerce application runs at the interval you specify during the Hanzo Commerce application's runtime. Your efforts are only spent on implementing the functionality performed by the job, such as syncing products to an ERP.

<Note title="Don't use scheduled jobs if" type="error">

- You want the action to execute at a specified schedule while the Hanzo Commerce application **isn't** running. Instead, create a [custom CLI script](../custom-cli-scripts/page.mdx) and execute it using the operating system's equivalent of a cron job.
...
![Example of scheduled job file in the application's directory structure](/images/learn/scheduled-job-dir-overview_ediqgm.jpg)

export const highlights = [
  ["3", "greetingJob", "The scheduled job function to execute at a specified interval."]
  ["3", "container", "Receive the Hanzo Commerce container as a parameter"],
...
You export an asynchronous function that receives the [Hanzo Commerce container](../hanzo-container/page.mdx) as a parameter. In the function, you resolve the [Logger utility](../../debugging-and-testing/logging/page.mdx) from the Hanzo Commerce container and log a message.
...
To test out your scheduled job, start the Hanzo Commerce application:
...
The next time you start the Hanzo Commerce application, it will run this job every day at midnight.
