...
You can protect API routes using the `authenticate` [middleware](../middlewares/page.mdx) from the Hanzo Commerce Framework. When applied to a route, the middleware checks that:
...
### Override Authentication for Hanzo Commerce's API Routes

In some cases, you may want to override the authentication requirement for Hanzo Commerce's API routes. For example, you may want to allow custom actor types to access existing protected API routes.
...
To access the authentication details in an API route, such as the logged-in user's ID, set the type of the first request parameter to `AuthenticatedHanzoRequest`. It extends `HanzoRequest`:

```ts highlights={[["7", "AuthenticatedHanzoRequest"]]}
import type {
  AuthenticatedHanzoRequest,
  HanzoResponse,
} from "@hanzo/commerce-framework/http"
...
The `auth_context.actor_id` property of `AuthenticatedHanzoRequest` holds the ID of the authenticated user or customer. If there isn't any authenticated user or customer, `auth_context` is `undefined`.
...
In this example, you retrieve the ID of the authenticated user, customer, or custom actor type from the `auth_context` property of the `AuthenticatedHanzoRequest` object.
...
You can access the logged-in customer’s ID in all API routes starting with `/store` using the `auth_context.actor_id` property of the `AuthenticatedHanzoRequest` object. You can then use [Query](../../module-links/query/page.mdx) to retrieve the customer details, or pass the ID to a [workflow](../../workflows/page.mdx) that performs business logic.
...
In this example, you retrieve the customer's ID and resolve Query from the [Hanzo Commerce container](../../hanzo-container/page.mdx).
...
You can access the logged-in admin user’s ID in all API routes starting with `/admin` using the `auth_context.actor_id` property of the `AuthenticatedHanzoRequest` object. You can then use [Query](../../module-links/query/page.mdx) to retrieve the user details, or pass the ID to a [workflow](../../workflows/page.mdx) that performs business logic.
...
In this example, you retrieve the admin user's ID and resolve Query from the [Hanzo Commerce container](../../hanzo-container/page.mdx).
