export const metadata = {
  title: `Configure Request Body Parser`,
}
...
By default, the body parser limits the request body size to `100kb`. If a request body exceeds that size, the Hanzo Commerce application throws an error.
...
You can't override the request body size limit for existing routes, such as Hanzo Commerce's Store and Admin API routes, due to how [middlewares are applied](../middlewares/page.mdx#how-are-middlewares-ordered-and-applied).
...
The `multer` package is available through the `@hanzo/commerce` package, so you don't need to install it. However, for better TypeScript support, install the `@types/multer` package as a development dependency:
...
import { uploadFilesWorkflow } from "@hanzo/commerce/core-flows"
...
To upload the files, you can use the [uploadFilesWorkflow](!resources!/references/hanzo-workflows/uploadFilesWorkflow), which uploads the files using the configured [File Module Provider](!resources!/infrastructure-modules/file).
