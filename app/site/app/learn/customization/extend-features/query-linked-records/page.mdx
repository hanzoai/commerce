...
export const metadata = {
  title: `Guide: Query Product's Brands`,
}

# {metadata.title}

In the previous chapters, you [defined a link](../define-link/page.mdx) between the [custom Brand Module](../../custom-features/module/page.mdx) and Hanzo Commerce's [Product Module](!resources!/commerce-modules/product), then [extended the create-product flow](../extend-create-product/page.mdx) to link a product to a brand.

In this chapter, you'll learn how to retrieve a product's brand (and vice-versa) in two ways: Using Hanzo Commerce's existing API route, or in customizations, such as a custom API route.
...
Hanzo Commerce's existing API routes accept a `fields` query parameter that allows you to specify the fields and relations of a model to retrieve. So, when you send a request to the [List Products](!api!/admin#products_getproducts), [Get Product](!api!/admin#products_getproductsid), or any product-related store or admin routes that accept a `fields` query parameter, you can specify in this parameter to return the product's brands.

<Note>

Learn more about using the `fields` query parameter to retrieve custom linked data models in the [Retrieve Custom Linked Data Models from Hanzo Commerce's API Routes](../../../fundamentals/api-routes/retrieve-custom-links/page.mdx) chapter.
...
You can also retrieve linked records using [Query](../../../fundamentals/module-links/query/page.mdx). Query allows you to retrieve data across modules with filters, pagination, and more. You can resolve Query from the Hanzo Commerce container and use it in your API route or workflow.
...
export const highlights = [
  ["11", "resolve", "Resolve Query from the Hanzo Commerce container."],
...
This adds a `GET` API route at `/admin/brands`. In the API route, you resolve Query from the Hanzo Commerce container. Query has a `graph` method that runs a query to retrieve data. It accepts an object having the following properties:
...
## Next Steps: Customize Hanzo Commerce Admin

Client applications, such as the Hanzo Commerce Admin dashboard, can now use brand-related features, such as creating a brand or setting the brand of a product.

In the next chapters, you'll learn how to customize the Hanzo Commerce Admin to show a product's brand on its details page, and to show a new page with the list of brands in your store.
