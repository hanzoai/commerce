...
After linking the [custom Brand data model](../../custom-features/module/page.mdx) and Hanzo Commerce's [Product Module](!resources!/commerce-modules/product) in the [previous chapter](../define-link/page.mdx), you'll extend the create product workflow and API route to allow associating a brand with a product.
...
- Consume the [productsCreated](!resources!/references/hanzo-workflows/createProductsWorkflow#productsCreated) hook of the [createProductsWorkflow](!resources!/references/hanzo-workflows/createProductsWorkflow), which is executed within the workflow after the product is created. You'll link the product with the brand passed in the `additional_data` parameter.
...
The [createProductsWorkflow](!resources!/references/hanzo-workflows/createProductsWorkflow) used in the [Create Product API route](!api!/admin#products_postproducts) has a `productsCreated` hook that runs after the product is created. You'll consume this hook to link the created product with the brand specified in the request parameters.

To consume the `productsCreated` hook, create the file `src/workflows/hooks/created-product.ts` with the following content:

![Directory structure after creating the hook's file.](/images/learn/brands-hook-dir-overview_ltwr5h.jpg)
...
import { createProductsWorkflow } from "@hanzo/commerce/core-flows"
import { StepResponse } from "@hanzo/commerce-framework/workflows-sdk"
import { Modules } from "@hanzo/commerce-framework/utils"
import { LinkDefinition } from "@hanzo/commerce-framework/types"
...
2. An object of properties related to the step's context. It has a `container` property whose value is the [Hanzo Commerce container](../../../fundamentals/hanzo-container/page.mdx) to resolve Framework and commerce tools.
...
![Diagram showcasing how the order of defining a link affects creating the link](/images/learn/remote-link-brand-product-exp_fhjmg4.jpg)
...
![Directory structure after adding the middelwares file](/images/learn/brands-middleware-dir-overview_uczos1.jpg)
...
Make sure to replace `{token}` with the token you received from the previous request, `shipping_profile_id` with the ID of a shipping profile in your application, and `{brand_id}` with the ID of a brand in your application. You can retrieve the ID of a shipping profile either from the Hanzo Commerce Admin, or the [List Shipping Profiles API route](!api!/admin#shipping-profiles_getshippingprofiles).

The request creates a product and returns it.

In the Hanzo Commerce application's logs, you'll find the message `Linked brand to products`, indicating that the workflow hook handler ran and linked the brand to the products.
