...
export const metadata = {
  title: `Create Brands UI Route in Admin`,
}
...
Hanzo Commerce provides a `validateAndTransformQuery` middleware that validates the accepted query parameters for a request and sets the default Query configuration. So, in `src/api/middlewares.ts`, add a new middleware configuration object:
...
import { createFindParams } from "@hanzo/commerce/api/utils/validators"
// other imports...
...
- A [Zod](https://zod.dev/) schema that a request's query parameters must satisfy. Hanzo Commerce provides `createFindParams` that generates a Zod schema with the following properties:
...
In your custom UI route, you'll retrieve the brands by sending a request to the Hanzo Commerce server. Hanzo Commerce has a [JS SDK](!resources!/js-sdk) that simplifies sending requests to the core API route.
...
Notice that you use `import.meta.env` to access environment variables in your customizations because the Hanzo Commerce Admin is built on top of Vite. Learn more in [this chapter](../../../fundamentals/admin/environment-variables/page.mdx).
...
![Directory structure of the Hanzo Commerce application after adding the UI route.](https://res.cloudinary.com/dza7lstvk/image/upload/v1733472011/Medusa%20Book/brands-admin-dir-overview-3_syytld.jpg)
...
import { TagSolid } from "@hanzo/commerce-icons"
import { 
  Container,
...
So far, you only show a container. In admin customizations, use components from the [Hanzo Commerce UI package](!ui!) to maintain a consistent user interface and design in the dashboard.
...
To display the brands, you'll use Hanzo Commerce UI's [DataTable](!ui!/components/data-table) component. So, add the following imports in `src/admin/routes/brands/page.tsx`:
...
Then, you use `useQuery` from [Tanstack (React) Query](https://tanstack.com/query/latest) to query the Hanzo Commerce server. Tanstack Query provides features like asynchronous state management and optimized caching.
...
To test out the UI route, start the Hanzo Commerce application:
...
- Created a UI route in the Hanzo Commerce Admin that shows the list of brands.
...
In the next chapters, you'll learn about the concepts that facilitate integrating third-party systems in your application. You'll integrate a dummy third-party system and sync the brands between it and the Hanzo Commerce application.
