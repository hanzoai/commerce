...
func TestDeposits_RequiresAuthHeader(t *testing.T) {
	r := NewStaticResolver(map[string]Tenant{
		"pay.": {
			Name:    "liquidity",
			Backend: BackendConfig{Kind: "bd", URL: "https://bd."},
		},
	})
	h := Deposits(r, stubForwarder(t))

	req := httptest.NewRequest(http.MethodPost, "http://pay./checkout/v1/deposits", strings.NewReader(`{"amount_cents":1000}`))
	req.Host = "pay."
...
func TestDeposits_ForwardsToTenantBackend(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "http://pay./checkout/v1/deposits",
	...
	if !strings.HasPrefix(captured.url, "https://bd./") {
		t.Errorf("url = %q, want prefix https://bd./", captured.url)
	}
...
func TestDeposits_FailsClosedOnMissingBackend(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "http://pay./checkout/v1/deposits", strings.NewReader(`{}`))
	req.Host = "pay."
