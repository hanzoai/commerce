...
func returnHostsFor(slug string) []string {
	...
	case "hanzo":
		return []string{
			...
			"https://hanzo.ai",
			"https://www.hanzo.ai",
			// hanzo.id — sign-up onboarding. Its plan step sends a new customer
			// here and completes only when checkout returns them to /onboarding
			// paid.
			"https://hanzo.id",
	...
	case "lux":
		return []string{
			...
			// Lux Tel — its site and console send the customer to pay.lux.tel
			// for a top-up or a plan and expect them back afterwards.
			"https://lux.tel",
			"https://www.lux.tel",
			"https://console.lux.tel",
		}
	case "zoo":
		return []string{
			"https://zoo.ngo",
