...
var SendTemplateAsync = delay.Func("send-template-email", func(ctx context.Context, template, toEmail, toName, subject string, vars ...mandrill.Var) {
...
var SendTransactionalSkully = delay.Func("send-skully-email", func(ctx context.Context, templateName, toEmail, toName, subject string, args ...interface{}) {
	req := mandrill.NewSendTemplateReq()
	req.AddRecipient(toEmail, toName)
...
// Helper that will render a template and send it as body for
// transactional-template email.
var SendTransactional = delay.Func("send-template-email", func(ctx context.Context, templateName, toEmail, toName, subject string, args ...interface{}) {
	req := mandrill.NewSendTemplateReq()
	req.AddRecipient(toEmail, toName)
