...
func makeTestProcessor(t *testing.T, handler http.Handler) (*StripeProcessor, *httptest.Server) {
	...
	api := &client.API{}
	api.Init("sk_test_REDACTED", &sgo.Backends{
		API:     backend,
		Uploads: backend,
	})
...
func TestNewProcessor_WithToken(t *testing.T) {
	sp := NewProcessor("sk_test_REDACTED", "whsec_abc")
	if sp == nil {
		t.Fatal("NewProcessor returned nil")
	}
	if sp.accessToken != "sk_test_REDACTED" {
		t.Fatalf("accessToken = %q, want sk_test_REDACTED", sp.accessToken)
	}
...
func TestNewSubscriptionProcessor(t *testing.T) {
	sp := NewSubscriptionProcessor("sk_test_REDACTED", "whsec_sub")
	if sp == nil {
...
func TestStripeProcessor_Charge_InvalidRequest_ZeroAmount(t *testing.T) {
	sp := NewProcessor("sk_test_REDACTED", "")
