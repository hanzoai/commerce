...
var _ = Describe("models/referrer/tasks", func() {
	Context("interval and delay calculation", func() {
		...
		It("should generate unique time-related task names", func() {
			...
			t := date(0, 51, 5, 33)
			interval := IntervalFromInstant(t, width)
			label := NameFromInterval(6 * time.Minute, interval, "test-id")
			Expect(label).To(Equal("processReferrals-test-id-360000000-from-2100-08-12T00:50:00Z-to-2100-08-12T01:00:00Z"))
		})
	})
})
