...
var _ = Describe("billing", Ordered, func() {
	...
	Context("Meter Events", func() {
		...
		It("Should get aggregated summary (sum)", func() {
			res := &summaryResponse{}
			cl.Get("/billing/meter-events/summary?meterId="+meterId+"&userId=acme/alice", res)
		...
		It("Should fail summary without meterId", func() {
			res := &ApiError{}
			cl.Get("/billing/meter-events/summary?userId=acme/alice", res)
	...
	Context("Credit Grants", func() {
		It("Should create a credit grant", func() {
			...
			Expect(res.Id).NotTo(BeEmpty())
			Expect(res.UserId).To(Equal("acme/alice"))
		...
		It("Should list grants for a user", func() {
			...
			cl.Get("/billing/credits?userId=acme/alice", res)
		...
		It("Should get credit balance", func() {
			...
			cl.Get("/billing/credit-balance?userId=acme/alice", res)
	...
	Describe("included monthly allotment", Ordered, func() {
		...
		Before(func() {
			if plan == "" {
				Skip("the published catalog credits no plan with included cloud credit")
			}
		})
	...
	It("Should grant nothing for a plan with no included allotment", func() {
		// A zero-allotment plan mints nothing. Which slug that is comes from the
		// catalog, and the rollup must agree it declares zero before the grant is
		// asserted to be a no-op.
		plan := allotmentPlan(false)
		Expect(plan).NotTo(BeEmpty())

		roll := &map[string]interface{}{}
		cl.Get("/billing/usage/rollup?user=acme/allotnone&plan="+plan, roll)
...
// allotmentPlan is the first catalog plan whose included cloud-credit allotment
// is positive (credited) or zero (!credited), or "" when there is none.
func allotmentPlan(credited bool) string {
	for _, p := range billingApi.StaticPlans() {
		if (billingApi.IncludedMonthlyCents(p.Slug) > 0) == credited {
			return p.Slug
		}
	}
	return ""
