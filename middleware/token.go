...
func DecodeToken(c *gin.Context, tokString string, validationFn func(token.Claims) error) (*organization.Organization, *app.App, *token.Claims, bool) {
	// Peek at claims to get the org so we can use secret to verify the key
	claims := token.Claims{}
	if err := jwt.Peek(tokString, &claims); err != nil {
		http.Fail(c, 403, "Unable to decode token.", err)
	...
	if claims.Type == token.Refresh || claims.Type == token.Access {
		// Verify token signature
		if err := jwt.Decode(tokString, org.SecretKey, token.Algorithm, &claims); err != nil {
	...
	if err := ap.GetById(claims.AppName); err != nil {
		http.Fail(c, 500, "App does not exist", err)
		return org, nil, nil, false
	}

	// Verify token signature
	if err := jwt.Decode(tokString, ap.SecretKey, token.Algorithm, &claims); err != nil {
		http.Fail(c, 403, "Unable to verify token.", err)
...
func IsAccessIssuerRevoked(c *gin.Context, claims *token.Claims) bool {
	// No issuer
...
func IsCustomerIssuerRevoked(c *gin.Context, org *organization.Organization, claims *token.Claims) bool {
	// No issuer
	if claims.Type != token.Customer {
		return false
	}
...
func TokenRequired(masks ...bit.Mask) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// Set Stuff
		switch claims.Type {
		case token.Access, token.Customer, token.Api, token.Refresh:
			c.Set("claims", claims)
		default:
...
func GetClaims(c *gin.Context) *token.Claims {
	return c.MustGet("claims").(*token.Claims)
}
...
func ApiKeyOrAccessTokenOnly(c *gin.Context) {
	claims := GetClaims(c)
	if !token.IsApi(*claims) && !token.IsAccess(*claims) {
