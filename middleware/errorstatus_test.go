...
func TestReturnedErrorKeepsItsStatus(t *testing.T) {
	...
	// The route composes BENEATH the group whose middleware guards it. zip v1.23
	// wraps a definition's own subtree rather than a URL prefix, so a route
	// registered on the app merely SHARES the prefix — the handler would answer
