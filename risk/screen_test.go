...
func TestScreen_AControlStillStopsAShadowDecision(t *testing.T) {
	...
	if _, err := Place(s, Placement{Subject: customer("c1"), Effect: control.Block, Rate: 0, Reason: "known mule"}); err != nil {
...
func TestScreen_ACertainBlockCostsNoScoringHop(t *testing.T) {
	...
	if _, err := Place(s, Placement{Subject: customer("c1"), Effect: control.Block, Rate: 0, Reason: "blocked"}); err != nil {
		t.Fatalf("place: %v", err)
	}
...
func TestScreen_ReserveHoldsTheExactShareOfAPayout(t *testing.T) {
	...
	if _, err := Place(s, Placement{Subject: Subject{Kind: KindMerchant, ID: "m1"}, Effect: control.Reserve, Rate: 2500, Reason: "new merchant"}); err != nil {
		t.Fatalf("place: %v", err)
	}
...
func TestScreen_TenantIsolation(t *testing.T) {
	...
	if _, err := Place(a, Placement{Subject: customer("shared-id"), Effect: control.Block, Rate: 0, Reason: "a's fraudster"}); err != nil {
...
func TestPlace_IsIdempotentWhileTheControlIsInForce(t *testing.T) {
	...
	first, err := Place(s, Placement{Subject: subject, Effect: control.Hold, Rate: 0, Reason: "review"})
	if err != nil {
		t.Fatalf("first: %v", err)
	}
	for i := 0; i < 5; i++ {
		again, err := Place(s, Placement{Subject: subject, Effect: control.Hold, Rate: 0, Reason: "review"})
		if err != nil {
...
func TestPlace_RefusesAnImpossibleReserveRate(t *testing.T) {
	...
	for _, rate := range []int64{0, -1, control.FullRate + 1} {
		if _, err := Place(s, Placement{Subject: Subject{Kind: KindMerchant, ID: "m1"}, Effect: control.Reserve, Rate: rate}); err == nil {
...
func TestPlace_RecordsTheAuthorFromThePrincipal(t *testing.T) {
	...
	c, err := Place(s, Placement{Subject: Subject{Kind: KindMerchant, ID: "m1"}, Effect: control.Hold, Reason: "review"})
	if err != nil {
		t.Fatalf("place: %v", err)
	}
	if c.By != "u_test" {
