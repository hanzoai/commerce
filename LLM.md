...
- Pre-existing debt: thirdparty/reamaze verifyHMAC has an inversion bug
...
- `LICENSE` — the dual offer + SPDX id + inbound=outbound contribution clause.
  ...
  (the real span: the code dates from 2014-09-29, when it was `kaching`).
...
`LICENSE-MIT` and `NOTICE`. It is the true span: the code dates from
...
## Public repository, one root

hanzoai/commerce is public and starts at a single root commit. The history
before it carried third-party credentials, customer data and old customer orgs,
so it was not published: every branch, archive ref and tag the repo used to
carry lives privately in `hanzo-inc/commerce` under `refs/archive/public/*`.

Test data is fictional and stays that way: the Such Tees store in
`models/fixtures`, `example.com` and `acme.test` addresses. Never a customer
org, a real person, a real address or a captured provider payload.
