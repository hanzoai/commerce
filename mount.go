...
func Mount(app *zip.App, deps cloud.Deps) error {
	...
	// The money plane's risk face, and ONLY it. The /v1 model bundle stays
	// unregistered for the reason written below — those prefixes are cloud's own
	// — but /v1/billing/risk has no other owner in either binary, and a face
	// nothing registers is a face nothing serves. It registers on the ROOT and
	// names its whole path in one literal, because the doc generator resolves an
	// op's identity from the group literal it can read (api/billing/risk.go).
