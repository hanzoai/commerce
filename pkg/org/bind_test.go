...
func cachedOrg(t *testing.T, name string) *organization.Organization {
	...
	o.CryptoDeposit = map[string]pricing.CryptoDeposit{"usdc": {FeeCents: 50}}
...
func TestBind_CopiesAreFullyIndependent(t *testing.T) {
	...
	a.CryptoDeposit["usdc"] = pricing.CryptoDeposit{FeeCents: 0}
	a.CryptoDeposit["btc"] = pricing.CryptoDeposit{FeeCents: 1}
	...
	for label, o := range map[string]*organization.Organization{"request B": b, "cache": cached} {
		...
		if o.CryptoDeposit["usdc"].FeeCents != 50 || len(o.CryptoDeposit) != 1 {
			t.Errorf("%s: CryptoDeposit aliased — a deposit fee is shared mutable state", label)
		}
	}
}
...
var referenceFields = []string{
	"Admins",        // cloned
	"CryptoDeposit", // cloned (a map of value structs, so a shallow map clone is a full copy)
