...
func TestPublishedTiersCarryLicensing(t *testing.T) {
	...
	want := map[string][]string{
		...
		"dev":        {"crm", "team"},
		"max-5x":     {"crm", "engine", "team"},
		"max-20x":    {"crm", "engine", "team"},
		"team":       {"crm", "team"},
		"enterprise": {"crm", "engine", "team"},
	}
...
func TestLicensingSurvivesRetirement(t *testing.T) {
	...
	if p.Licensing == nil || !slices.Equal(p.Licensing.Products, []string{"crm", "team"}) {
		t.Fatalf("retired plan %q licenses %v, want [crm team] — a retired tier must not lose its licences", "dev", p.Licensing)
	}
}
