...
// declared credit. This is RED's intended behavior — "an org
...
func TestAllotment_OrgAdminMatchingSubscriptionHonored(t *testing.T) {
	...
	if amt, _ := out["amountCents"].(float64); int64(amt) != want {
		t.Fatalf("org-admin with a real 'team' subscription: amountCents=%v, want %d (clamp honors the matching paid plan)", amt, want)
	}
}
...
func TestAllotment_PlatformAppMayNameAnyPlan(t *testing.T) {
	...
	if amt, _ := out["amountCents"].(float64); int64(amt) != want {
		t.Fatalf("service-token named 'team': amountCents=%v, want %d (privileged override honored)", amt, want)
	}
}
...
// mintingTeam gives team a $5-a-seat allotment for the life of one test and
// returns what it mints, in cents. The published catalog mints no credit — a
// plan covers AI instead — so the grant's clamp is exercised on a fixture rung.
func mintingTeam(t *testing.T) int64 {
	t.Helper()
	p := lookupPlan("team")
	if p == nil || p.Limits == nil {
		t.Fatal("team missing from the catalog")
	}
	was := p.Limits.IncludedCloudCreditsPerUser
	five := 5
	p.Limits.IncludedCloudCreditsPerUser = &five
	t.Cleanup(func() { p.Limits.IncludedCloudCreditsPerUser = was })
	return IncludedMonthlyCents("team")
}
