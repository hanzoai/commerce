...
type TierView struct {
	...
	// Plan is the catalog rung the subject is served as, or "" when they hold no
	// subscription. A tier names a class of plan (every paid rung is Pro); a bound
	// that differs between rungs of one class — the rolling spend cap Max
...
func TierOf(ctx context.Context, org *organization.Organization, user string) (tier.Name, error) {
	...
	return deriveTier(datastore.New(org.Namespaced(ctx)), user, org.TestMode())
}
...
//
// A plan the catalog does not publish is the other case (activeTier): a private
// plan made for one customer lives only in the plan authority, so what it was
// sold at is read from the snapshot the subscription carries.
//
// Fail-safe: the query error is returned, not collapsed to Free.
func deriveTier(db *datastore.Datastore, user string, test bool) (tier.Name, error) {
...
//
// A slug the catalog publishes answers from the catalog (tierForActivePaidSlug),
// which no edit to a row or a snapshot can move. A plan only the plan authority
// holds answers from the price the subscription was sold at (paidRow), and only on
// a row a payment opened: the creation gate scores the catalog, so it cannot vouch
// for a plan the catalog does not know, and a self-created internal row on one must
// not confer a tier nobody paid for.
func activeTier(s *subscription.Subscription) tier.Name {
	slug := subscriptionSlug(s)
	if lookupPlan(slug) != nil {
		return tierForActivePaidSlug(slug)
	}
	if !paidRow(s) || !subscriptionPaymentBacked(s) {
		return tier.Free
	}
	if s.Plan.Category == "enterprise" {
		return tier.Enterprise
	}
	return tier.Pro
}
