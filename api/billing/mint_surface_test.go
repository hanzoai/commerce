...
func routeMintSurface(v1 *zip.Group) {
...
func registeredMintRoutes(t *testing.T, reaches map[string]bool) []mintRoute {
	...
	app.Use(zip.H(func(c *zip.Ctx) error {
		...
		c.Locals("organization", &organization.Organization{Name: "acme"})
		return c.Next()
	}))

	type seen struct{ method, path, handler string }
	var found []seen
	probe := app.Group("/v1").With(func(next zip.Handler) zip.Handler {
		return func(c *zip.Ctx) error {
			found = append(found, seen{
				method:  c.Method(),
				path:    c.Path(),
				handler: runtime.FuncForPC(reflect.ValueOf(next).Pointer()).Name(),
			})
			return c.NoContent(http.StatusTeapot)
		}
	})
	routeMintSurface(probe)
