...
func TestF1_PatchUpgrade_NonMint_Gated(t *testing.T) {
	...
	// A payment-backed sub on a rung that grants NO allotment (what POST
	// /subscribe/card leaves after a real Square charge on "pro").
	...
	s.Plan.Slug = "pro"
	s.PlanId = "pro"
	s.Quantity = 2 // the granting rung is per-seat and has a 2-seat minimum
	if err := s.Create(); err != nil {
		t.Fatalf("seed sub: %v", err)
	}

	// A NON-mint org owner PATCHes it up to "team" (allotment $100 ≫ pro's none).
	// Before the fix this succeeded free; now it is 403.
	r := invokeSubPatch(org, ctx, c1OrgAdmin, s.Id(), `{"planId":"team"}`)
	if r.StatusCode != http.StatusForbidden {
		t.Fatalf("non-mint PATCH pro→team: status=%d body=%s, want 403 (allotment-increase gated)", r.StatusCode, bodyOf(r))
	}

	// The sub was NOT upgraded — a later allotment/grant anchors on pro (none),
	// never team ($100). No spendable credit was laundered.
	...
	if r2 := invokeSubPatch(org, ctx, c1MintPrincipal, s2.Id(), `{"planId":"team"}`); r2.StatusCode == http.StatusForbidden {
		t.Fatalf("mint-principal PATCH pro→team: status=403 body=%s, want allowed (authorized mint principal)", bodyOf(r2))
	}
}
...
func TestF2_DBPriceZero_StillGatedByEmbedPaidTier(t *testing.T) {
	...
	// Seed the authority, then SuperAdmin-edit "max-5x" DB Price → 0.
	if _, _, err := SeedPlans(ctx); err != nil {
		t.Fatalf("seed: %v", err)
	}
	adb := plan.AuthorityDB(ctx)
	p := plan.New(adb)
	if ok, _ := p.Query().Filter("Slug=", "max-5x").Get(); !ok {
		t.Fatal("max-5x not seeded")
	}
	p.Price = 0
	...
	rp, err := resolveSubscriptionPlan(odb, "max-5x")
	if err != nil {
		t.Fatalf("resolve: %v", err)
	}
	if rp.Price != 0 {
		t.Fatalf("DB edit did not land: resolve(max-5x).Price = %d, want 0", rp.Price)
	}

	// Yet the C1-a gate STILL refuses a non-mint org owner — it reads the embed
	// paidTier("max-5x"), not the DB Price=0. No free provision of the paid tier.
	w := invokeSub(org, ctx, c1OrgAdmin, CreateBillingSubscription, `{"userId":"acme/self","planId":"max-5x"}`)
...
// seedAndGrantingHashid seeds the plan authority and returns the granting rung's DB HASHID — the
...
// seedAndGrantingHashid returns the DB hashid of the rung that grants an
// allotment — the only kind of move the F1 gate has anything to score.
func seedAndGrantingHashid(t *testing.T, ctx context.Context) string {
	t.Helper()
	if _, _, err := SeedPlans(ctx); err != nil {
		t.Fatalf("seed: %v", err)
	}
	tm := plan.New(plan.AuthorityDB(ctx))
	if ok, _ := tm.Query().Filter("Slug=", "team").Get(); !ok {
		t.Fatal("team not seeded")
	}
	id := tm.Id()
	if id == "" {
		t.Fatal("team has no hashid")
	}
	return id
}

// TestR1_PatchUpgrade_HashidForm_Gated is Red R1 (F1 half): the F1 allotment gate
// must score the RESOLVED slug, so passing "team"'s DB HASHID gates IDENTICALLY to
...
func TestR1_PatchUpgrade_HashidForm_Gated(t *testing.T) {
	...
	hashid := seedAndGrantingHashid(t, c)
	...
	if rp.Slug != "team" {
		t.Fatalf("hashid resolved to %q, want team — test not exercising R1", rp.Slug)
	}
	...
	if r.StatusCode != http.StatusForbidden {
		t.Fatalf("non-mint PATCH pro→team(HASHID): status=%d body=%s, want 403 (hashid must not bypass the gate)", r.StatusCode, bodyOf(r))
	}
