...
//
// The facts it reads are all server-minted and stripped from client input at the
// edge (gateway/iamauth MintedIdentityHeaders), so none is forgeable:
//
//	org                  — the resolved org (middleware), the ledger money lives in
...
func payer(c *zip.Ctx) account.Account {
	...
	return account.Payer(account.Credential{
		Owner:   org,
		Name:    c.Header("X-User-Id"),
		Account: c.Header("X-Billing-Account-Id"),
	})
}
