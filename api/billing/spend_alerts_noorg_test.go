...
//
// On the co-resident cloud embed path a browser read of GET /v1/billing/alerts
...
func TestSpendAlerts_NoOrgInContext_NoPanic(t *testing.T) {
	...
	t.Run("list is honest-empty, never a panic→502", func(t *testing.T) {
		...
		w := driveSeeded(noOrg, "/v1/billing/alerts", req, ListSpendAlerts)
		if w.StatusCode != 200 {
	...
	t.Run("create is a clean 401, never a panic→502", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodPost, "/v1/billing/alerts",
			strings.NewReader(`{"threshold":100}`))
		req.Header.Set("Content-Type", "application/json")
		w := driveSeeded(noOrg, "/v1/billing/alerts", req, CreateSpendAlert)
		if w.StatusCode != 401 {
	...
	t.Run("update is a clean 401, never a panic→502", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodPatch, "/v1/billing/alerts/abc",
			strings.NewReader(`{"title":"x"}`))
		req.Header.Set("Content-Type", "application/json")
		w := driveSeeded(noOrg, "/v1/billing/alerts/:id", req, UpdateSpendAlert)
		if w.StatusCode != 401 {
	...
	t.Run("delete is a clean 401, never a panic→502", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodDelete, "/v1/billing/alerts/abc", nil)
		w := driveSeeded(noOrg, "/v1/billing/alerts/:id", req, DeleteSpendAlert)
		if w.StatusCode != 401 {
