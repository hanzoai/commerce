...
// self-create a PAID-tier ("max-5x": $100/mo, $25/mo included allotment) Active
...
func TestCreateSubscription_OrgAdminPaidTier_Denied(t *testing.T) {
	...
	body := `{"userId":"acme/self","planId":"max-5x"}`
...
func TestSubscriptionPlanSlug_PaymentBackedOnly(t *testing.T) {
	...
	if got := subscriptionPlanSlug(db, "acme/forger", false); got != "" {
	...
	if got := subscriptionPlanSlug(db, "acme/payer", false); got != "max" {
	...
	if got := subscriptionPlanSlug(db, "acme/invoiced", false); got != "max" {
	...
	if got := subscriptionPlanSlug(db, "acme/freeuser", false); got != "developer" {
...
func TestPlanForGrant_OrgAdminCannotAnchorOnForgedSub(t *testing.T) {
	...
	if got := planForGrant(c, db, "acme/forger", "max", false); got != "" {
