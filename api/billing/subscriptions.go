...
type createSubscriptionRequest struct {
	...
	// Test is the org's mode at the sale, stamped on the row and its children. It
	// is set by the endpoint from the org, never read off the wire.
	Test bool `json:"-"`
	// Collected opens the row as a plan paid OUTSIDE checkout, on the period that
	// payment covers. Only RecordSubscription sets it, never a request body.
...
func CreateBillingSubscription(c *zip.Ctx) error {
	...
	//
	// A plan the catalog does not publish has no embed entry to score, so it is scored
	// by its own row (paidPlan), which is what the tier reads from the snapshot. Left
...
// it is $198.96, four cents short on every renewal.
//
// The identity holding the two halves together is that MonthlyNormalizedCents
// reads AnnualTotal straight back to PriceAnnual: the price the revenue board
// reports is the price the catalog advertises, and neither side knows the other
// exists.
//
// A plan carrying no annual total is not sold annually. The free tier's is $0 and
// a contact-sales tier has no price at all, so a year of it is a $0 charge —
...
// single $999 year, so Max at level 3 has a published monthly price ($299) and no
...
func createSubscription(db *datastore.Datastore, p *plan.Plan, req *createSubscriptionRequest) (*subscription.Subscription, error) {
	...
	for _, childSlug := range bundled {
		...
		childSub.Type = sub.Type
		childSub.Quantity = 1
		childSub.Test = sub.Test
...
func provisionMembers(db *datastore.Datastore, parent *subscription.Subscription, planSlug string, base *plan.Plan, members []string) {
	...
	for _, m := range members {
		...
		childSub.ProviderType = "bundle"
		childSub.Type = parent.Type
		childSub.Quantity = 1
		childSub.Test = parent.Test
...
//
// The org's namespace is the whole scope. Subscriptions are stored without a
// parent key (see subscriptionPlanSlug), so a synckey ancestor filter matched no
// row at all and every account's list read empty, its MRR zero.
...
func UpdateBillingSubscription(c *zip.Ctx) error {
	...
	if planChanged {
		...
		// whose payment was for the plan it leaves. So a move onto one is scored by
		// its own row (paidPlan) and is a staff move, as opening one is.
