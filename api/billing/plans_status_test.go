...
func TestPlanStatus_ArchivedPlanRefusedBeforeCardCharge(t *testing.T) {
	...
	p, err := resolveSubscriptionPlan(db, "max-5x")
	if err != nil {
...
func TestPaidTier_ContactSalesIsNotSelfServe(t *testing.T) {
	...
	// max is retired and answered by max-5x, so a holder of it is still paid.
	for _, slug := range []string{"dev", "max-5x", "max-20x", "team", "max"} {
		if !paidTier(slug) {
