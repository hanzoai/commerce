...
// nothing forces to agree. The catalog states a year — Dev's $199 — and shows it
// per month as $16.58, while Price is collected in FULL once per period, and a
// yearly period is a year. So the charge is the stated year and the revenue
// report is that year / 12, and if either side picked a different figure the other
// would keep working: a year of Dev sold for twelve rounded months ($198.96)
// still renews, still invoices, still reports. These pin both directions and the
// identity between them.

// annual reads a plan's published prices, so these tests assert the mechanism
...
func annual(t *testing.T, slug string) (monthly, perMonth, year currency.Cents) {
	t.Helper()
	sp := lookupPlan(slug)
	if sp == nil {
...
// plan states, an absent choice changes nothing, and a period the plan does not
// publish is refused rather than approximated.
func TestPlanAtInterval(t *testing.T) {
	p := &plan.Plan{
		Slug:          "dev",
		Price:         currency.Cents(1900),
		PriceAnnual:   currency.Cents(1658),
		AnnualTotal:   currency.Cents(19900),
	...
	if year.Price != 19900 {
		t.Errorf("a year costs %d, want the stated 19900 (twelve rounded months would be %d)", year.Price, 1658*12)
	}
	...
	for name, none := range map[string]*plan.Plan{
		...
		// The per-month tag alone is not a year. Multiplying it back up is the
		// four-cent shortfall this field exists to end, so it is not sold at all.
		"an annual tag and no year": {
			Slug: "tagged", Price: currency.Cents(1900), PriceAnnual: currency.Cents(1658), Interval: types.Monthly,
		},
	} {
...
func TestAnnualNormalizesToTheAdvertisedPrice(t *testing.T) {
	...
	for _, sp := range catalog {
		...
		if err != nil {
			t.Errorf("%s publishes a %d-cent year but is not sold annually: %v", sp.Slug, sp.AnnualTotal, err)
			continue
		}
		sold++

		if int64(year.Price) != sp.AnnualTotal {
...
func TestAnnualAtALevelIsRefused(t *testing.T) {
	maxLevels(t)
	row := planFromStatic(lookupPlan("max-5x"))
	...
	if base.Price != row.AnnualTotal {
		t.Fatalf("max annual = %d, want the stated %d", base.Price, row.AnnualTotal)
	}
...
// charged the year the catalog states, once, and the subscription carries that
// amount for every renewal after it.
func TestSubscribeWithCardChargesTheWholeYear(t *testing.T) {
	monthly, perMonth, stated := annual(t, "dev")
	want := int64(stated)
	...
	if int64(out["amountCents"].(float64)) != want {
		t.Fatalf("receipt amountCents=%v, want the stated year %d", out["amountCents"], want)
	}
...
func TestSubscribeWithCardRefusesAnnualWithoutAnAnnualPrice(t *testing.T) {
	...
	cases := map[string]string{
		...
		"annual at a level":             `{"sourceId":"cnon:ok","planId":"max-5x","level":3,"interval":"year"}`,
	}
	...
	for name, body := range cases {
		t.Run(name, func(t *testing.T) {
			...
			db := datastore.New(org.Namespaced(ctx))
			for _, slug := range []string{"free", "enterprise", "dev", "max-5x"} {
