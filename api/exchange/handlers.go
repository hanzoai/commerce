...
func Confirm(c *zip.Ctx) error {
	return transition(c, exchangeModel.StatusConfirmed, "confirm")
}

// Cancel transitions an open exchange to canceled.
func Cancel(c *zip.Ctx) error {
	return transition(c, exchangeModel.StatusCanceled, "cancel")
}
...
func transition(c *zip.Ctx, target, verb string) error {
	...
	if err := e.GetById(id); err != nil {
		return http.Fail(c, 404, "No exchange found with id: "+id, err)
	}

	if !e.IsOpen() {
		return http.Fail(c, 409, "Cannot "+verb+" exchange in status: "+e.Status, errors.New("exchange not open"))
	}

	e.Status = target
	if err := e.Update(); err != nil {
		return http.Fail(c, 500, "Failed to "+verb+" exchange", err)
	}
