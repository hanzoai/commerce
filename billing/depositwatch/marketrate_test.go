...
// A native coin has no contract, so the value is not read. The key is the whole
// declaration.
func TestANativeCoinNeedsNoContract(t *testing.T) {
	got, err := AssetsFromEnv([]string{
		"CRYPTO_DEPOSIT_RPC_BITCOIN=https://esplora",
		"CRYPTO_DEPOSIT_TOKEN_BITCOIN_BTC=1",
	})
	if err != nil {
		t.Fatalf("AssetsFromEnv: %v", err)
	}
	if len(got) != 1 || !got[0].MarketPriced() {
		t.Fatalf("got %+v, want one market-priced asset", got)
	}
}
