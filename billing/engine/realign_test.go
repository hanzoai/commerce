...
// current invoice's PeriodEnd is moved onto that invoice's period. A dry run
// answers the move and writes nothing; the real run writes it; a second run
// finds nothing to do.
func TestRealign_MovesARowBackOntoItsPaidInvoice(t *testing.T) {
	...
	for _, p := range []plan.Plan{monthly, yearly} {
		t.Run(string(p.Interval), func(t *testing.T) {
			...
			dry, err := Realign(db, reload(t, db, sub.Id()), true)
			if err != nil || dry == nil {
			...
			done1, err := Realign(db, reload(t, db, sub.Id()), false)
			...
			if again, err := Realign(db, got, false); err != nil || again != nil {
...
func TestRealign_LeavesEveryOtherRowAlone(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"on its paid period", func(t *testing.T, db *datastore.Datastore) *subscription.Subscription {
		...
		{"renewed onto its paid period", func(t *testing.T, db *datastore.Datastore) *subscription.Subscription {
		...
		{"its invoice is not paid", func(t *testing.T, db *datastore.Datastore) *subscription.Subscription {
			sub := oldShape(t, db, monthly, t0)
		...
		{"its invoice bills no period", func(t *testing.T, db *datastore.Datastore) *subscription.Subscription {
			sub := oldShape(t, db, monthly, t0)
			inv, err := loadInvoice(db, sub.CurrentInvoiceId)
			if err != nil {
				t.Fatal(err)
		...
		{"its invoice is another subscription's", func(t *testing.T, db *datastore.Datastore) *subscription.Subscription {
			sub := oldShape(t, db, monthly, t0)
			inv, err := loadInvoice(db, sub.CurrentInvoiceId)
			if err != nil {
				t.Fatal(err)
		...
		{"it names no invoice", func(t *testing.T, db *datastore.Datastore) *subscription.Subscription {
			sub := oldShape(t, db, monthly, t0)
		...
		{"it has ended", func(t *testing.T, db *datastore.Datastore) *subscription.Subscription {
			sub := oldShape(t, db, monthly, t0)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			sub := tc.shape(t, db)
			before := reload(t, db, sub.Id())
			if r, err := Realign(db, before, false); err != nil || r != nil {
...
func TestRealign_NeverRunsInsideTheCycle(t *testing.T) {
	...
	if _, err := Realign(db, reload(t, db, sub.Id()), false); err != nil {
